...
void AAUChinaRealName::ManualVerify(const FString& UserID, const FString& Name, const FString& CardID, bool bFallback,
	...
	AAUNet::ChinaManualVerify(UserID, Name, CardID, bFallback, [=](TSharedPtr<FAAURealNameResultModel> ModelPtr, const FTUError& Error) {
		if (!CallBack) {
			return;
		}
		if (ModelPtr.IsValid()) {
